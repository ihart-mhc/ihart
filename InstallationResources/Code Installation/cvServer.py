...
class Data:
    ...
    #                their nature, not two areas of motion will actually overlap--if they
    ...
    quitTrackbar = "quit"
    resetTrackbar = "reset interest areas"
    lastResetValue = 0                      # areas of interest are reset when the trackbar changes position.
    ...
    def updateTrackbars(self, x):
        ...
        # If the reset interest areas trackbar is different from its last position,
        # resent the areas of interest.
        reset = cv2.getTrackbarPos(self.resetTrackbar, self.gWindow)
        if self.lastResetValue != reset:
            self.resetAreas()
            self.lastResetValue = reset
    ...
    def createGUI(self):
        ...
        cv2.createTrackbar(self.resetTrackbar, self.gWindow, self.lastResetValue, 1, self.updateTrackbars)
