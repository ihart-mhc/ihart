...
*
**/
void checkTypeEnabled(StringBuffer s){
...
**/
String createOutputString(Blob[] blobs, int type){
...
**/
void createBlobCentroid(Blob blob){
...
**/
void filterResume(StringBuffer s){
...
**/
String filterTest( Blob b, int indexOfInterest, int numBlobbers, int type ){
   //calculate the scaled coordinates for the blob
 ...
 **/
double[] calcScaled( float x, float y, float wid, float hei, int areaOfInterestIndex )
{
...
**/
float getFloatOut( ArrayList list, int index )
{
...
**/
void boundArea(float start,float end, float w, float h){
  noFill();
  stroke(255, 0, 0);
  rect(start, end, w, h);
}
...
**/
void fillFollowed(Blob b, int type){
...
**/
Polygon createPolygon(Blob blob){
...
**/
boolean isWhite(Blob blob, Polygon p, int[] pix){
...
void mouseDragged() {
   ...
   if(shellThresholding){
     shellThresh = int( map(mouseY,0,height,0,255) );
     thresh = true;
   }
...
**/
boolean pointBtw(double xLeft, double xRight, double yTop, double yBottom, double pointX, double pointY){
...
**/
void addAreaOfInterest( float startX, float startY, float endX, float endY )
{
   //bound new area
